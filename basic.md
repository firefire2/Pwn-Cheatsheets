...
作成したパターンを、exploitコード[exploit-template.py](https://github.com/firefire2/BufferOverflow-Cheatsheets/blob/main/exploit-template.py)のpayloadとして追記し、プログラムに送り込む。
...
これらを順番に確認して、飛んでいる数字がBadcharである。ただし、1つとは限らないため、1つ特定する度にexploitのpayloadから該当するBadcharを削除して、再度プログラムに送り込んで確認する。
...
## シェルコードの実行
実行したいシェルコードの先頭をEIPとして設定するには、スタックポインタが示すアドレスにプログラムの実行を飛ばすコードがあるアドレス(=JMP ESPのアドレス)をEIPに設定すればよい。
